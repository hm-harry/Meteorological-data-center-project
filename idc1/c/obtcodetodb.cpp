/*
    obtcodetodb.cpp本程序用于把全国站点参数数据保存到数据库T_ZHOBTCODE表中
    作者：吴惠明
*/
#include "_public.h"
#include "_mysql.h"
...
vector<struct st_stcode> vstcode; // 存放气象站点参数的容器

// 把全国站点参数文件加载到容器vstcode容器中
...
int main(int argc, char* argv[]){
    ...
    if(argc != 5){
        ...
        printf("Example:/project/tools1/bin/procctl 120 /project/idc1/bin/obtcodetodb /project/idc1/ini/stcode.ini \"127.0.0.1,root,whmhhh1998818,mysql,3306\" utf8 /log/idc/obtcodetodb.log\n\n");
        printf("本程序用于把全国站点参数数据保存到数据库中，如果站点不存在则插入，站点已存在则更新。\n");
    ...
    stmtins.prepare("insert into T_ZHOBTCODE(obtid, cityname, provname, lat, lon, height, uptime) values(:1, :2, :3, :4*100, :5*100, :6*10, now())");
    ...
    stmtupt.prepare("update T_ZHOBTCODE set cityname = :1, provname = :2, lat = :3*100, lon = :4*100, height = :5*10, uptime = now() where obtid = :6");
...
bool LoadSTCode(const char* inifile){
    ...
    while(true){
        ...
        CmdStr.GetValue(1, stcode.obtid, 10);    // 站点
