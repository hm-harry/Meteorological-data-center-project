...
int main(int argc, char* argv[]){
  ...
  if((argc != 5) && (argc != 6)){
    printf("Using:./crtsurfdatal inifile outpath logfile datafmd [datetime]\n");
    printf("Example:/project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/sufdata /log/idc/crtsurfdata.log xml,json,csv\n");
    printf("        /project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/sufdata /log/idc/crtsurfdata.log xml,json,csv 20220711123000\n\n");
    printf("        /project/tools1/bin/procctl 60 /project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/sufdata /log/idc/crtsurfdata.log xml,json,csv\n\n\n");
  ...
  // 关闭全部的信号和输入输出
  // 设置信号，在shell状态下可以用kill+进程号正常终止进程
  ...
  for(int ii = 0; ii < vsurfdata.size(); ++ii){
    ...
    if(strcmp(datafmt, "json") == 0){
      File.Fprintf("{\"obtid\":\"%s\",\"ddatetime\":\"%s\",\"t\":\"%.1f\","\
			    "\"p\":\"%.1f\",\"u\":\"%d\",\"wd\":\"%d\",\"wf\":\"%.1f\",\"r\":\"%.1f\",\"vis\":\"%.1f\"}", \
                      vsurfdata[ii].obtid, vsurfdata[ii].ddatetime, vsurfdata[ii].t/10.0, vsurfdata[ii].p/10.0, vsurfdata[ii].u, vsurfdata[ii].wd\
                      , vsurfdata[ii].wf/10.0, vsurfdata[ii].r/10.0, vsurfdata[ii].vis/10.0);
