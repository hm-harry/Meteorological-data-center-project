...
int main(int argc,char *argv[])
{
  ...
  if(oraconnpool.init(starg.connstr, starg.charset, 10 ,50)  == false){// 初始化数据库连接池
    ...
    if(pthread_create(&checkpoolid, NULL, checkpool, 0) != 0){ // 多开一个进程用来定期检查数据库连接池
  ...
  for(int ii = 0; ii < 10; ++ii)
  {
    ...
    stpthinfo.atime = time(0); // 把线程活动的时间设置为当前时间
    vthid.push_back(stpthinfo); // 把线程的id保存到容器中
  }
...
// 以下代码是为了防止信号处理函数在执行的过程中被信号中断
  ...
  pthread_cancel(checkpthid); // 取消线程监控
  pthread_cancel(checkpoolid); // 取消检查数据库池的代码
