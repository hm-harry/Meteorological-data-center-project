...
int main(int argc, char* argv[]){
  ...
  if(argc != 4){
    ...
    printf("本程序把pathname目录及子目录中timeout天之间的匹配matchstr文件全部压缩，timeout可以是小数。\n");
  ...
  while(true){
    ...
    if((strcmp(Dir.m_ModifyTime, strTimeOut) < 0) && (MatchStr(Dir.m_FileName, "*.gz") == false)){
      // 压缩文件，调用操作系统的gzip命令
      SNPRINTF(strCmd, sizeof(strCmd), 1000, "/usr/bin/gzip -f %s 1>/dev/null 2>/dev/null", Dir.m_FullFileName);//1>/dev/null 2>/dev/null表示把标准输出和标准输入指向空
      if(system(strCmd) == 0){
        printf("gzip %s ok.\n", Dir.m_FullFileName);
      }else{
