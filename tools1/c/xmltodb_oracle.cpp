...
int totalcount, inscount, uptcount; // xml文件的总记录数，插入记录数，更新记录数
...
void _help(char *argv[]){
    ...
    printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/xmltodb_oracle /log/idc/xmltodb_oracle_vip2.log \
    \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><inifilename>/project/tools1/ini/xmltodb.xml</inifilename>\
    <xmlpath>/idcdata/xmltodb/vip2</xmlpath><xmlpathbak>/idcdata/xmltodb/vip2bak</xmlpathbak><xmlpatherr>/idcdata/xmltodb/vip2err</xmlpatherr>\
    <timetvl>5</timetvl><timeout>50</timeout><pname>xmltodb_oracle_vip2</pname>\"\n\n");
...
void crtsql(){
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    if (strcmp(TABCOLS.m_vallcols[ii].colname,"keyid")==0){
      SNPRINTF(strtemp,100,sizeof(strtemp),"ZEQ_%s.nextval",stxmltotable.tname+2);
    }else{
