...
int imaxfieldlen = -1;   // 结果集字段值的最大长度
...
struct st_arg{
  ...
  char pname[51];           // 进程名，建议使用"dminingoracle_后缀"的方式
}starg;
...
void _help(){
    ...
    printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE1.log \
    \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><charset>Simplified Chinese_china.AL32UTF8</charset><selectsql>Select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE1</selectsql>\
    <fieldstr>obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE1</bfilename>\
    <efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><timeout>30</timeout><pname>dminingoracle_ZHOBTCODE1</pname>\"\n\n");

    printf("     /project/tools1/bin/procctl 30 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND1.log \
    \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><charset>Simplified Chinese_china.AL32UTF8</charset>\
    <selectsql>Select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND1 where keyid>:1 and ddatetime>sysdate-0.04</selectsql>\
    <fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND1</bfilename>\
    <efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><starttime></starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/dminingoracle_ZHOBTMIND1_HYCZ.list</incfilename>\
    <timeout>30</timeout><pname>dminingoracle_ZHOBTMIND1_HYCZ</pname><maxcount>1000</maxcount><connstr1>qxidc/qxidcpwd@snorcl11g_132</connstr1>\"\n\n");
...
bool _xmltoarg(char *strxmlbuff){
  ...
  for(int ii = 0; ii < CmdStr.CmdCount(); ++ii){
    ...
    if(ifieldlen[ii] > imaxfieldlen) imaxfieldlen = ifieldlen[ii]; // 字段的长度不能超过imaxfieldlen
  }
...
bool  _dminingoracle(){
  ...
  char strfieldvalue[ifieldcount][imaxfieldlen + 1];// 抽取数据的SQL执行后存放结果集字段值的数组
