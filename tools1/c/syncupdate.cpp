...
void _help(char* argv[]){
    ...
    printf("       /project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate /log/idc/syncupdate_ZHOBTMIND2.log \"<localconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTMIND1</fedtname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where>where ddatetime>timestampadd(minute,-120,now())</where><synctype>2</synctype><synctype>2</synctype><remoteconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTMIND2</pname>\"\n\n");
    ...
    printf("where        同步数据的条件，为空表示同步全部的记录，填充在delete本地表和select Federated表之后。注意：1）where中的字段必须同时在本地表和Federated表中；2）不要用系统时间作为条件。\n");
...
bool _syncupdate(){
  ...
  if(starg.synctype == 1){
    ...
    stmtins.prepare("insert into %s(%s) select %s from %s %s\n", starg.localtname, starg.localcols, starg.remotecols, starg.fedtname, starg.where);
