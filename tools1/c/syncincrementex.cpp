/*
  程序名：syncincrementex.cpp，本程序是数据中心的公共功能模块，采用增量的方式同步MySQL数据库之间的表
...
void _help(char* argv[]){
    ...
    printf("Sample:/project/tools1/bin/procctl 10 /project/tools1/bin/syncincrementex /log/idc/syncincrement_ZHOBTMIND2.log \"<localconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</localconnstr><remoteconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</remoteconnstr><charset>utf8</charset><remotetname>T_ZHOBTMIND1</remotetname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><timetvl>2</timetvl><timeout>50</timeout><pname>syncincrement_ZHOBTMIND2</pname>\"\n\n");

    printf("       /project/tools1/bin/procctl 10 /project/tools1/bin/syncincrementex /log/idc/syncincrement_ZHOBTMIND3.log \"<localconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</localconnstr><remoteconnstr>172.29.193.250,root,whmhhh1998818,mysql,3306</remoteconnstr><charset>utf8</charset><remotetname>T_ZHOBTMIND1</remotetname><localtname>T_ZHOBTMIND3</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where>and obtid like '54%%%%'</where><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><timetvl>2</timetvl><timeout>50</timeout><pname>syncincrement_ZHOBTMIND3</pname>\"\n\n");

    printf("本程序是数据中心的公共功能模块，采用增量的方式同步MySQL数据库之间的表。\n");
    ...
    printf("where        同步数据的条件，填充在select remotekeycol from remotetname where remotekeycol > :1之后，\n"\
                          "注意不加where关键字。\n");
    printf("remoteconnstr远程数据库的连接参数，格式与localconnstr相同。\n");
    printf("remotetname  远程表名。\n");
    printf("remotekeycol 远程表的键值字段名。\n");
    printf("localkeycol  本地表的键值字段名。\n");

    printf("timetvl      执行同步是时间间隔，单位：秒，取值1-30。\n");
...
bool _syncincrementex(bool& bcontinue){
  ...
  // 从本地表starg.;ocaltname中获取自增字段的最大值，存放在maxkeyvalue全局变量中
  ...
  char colvalues[colcount][TABCOLS.m_maxcollen + 1]; // 从远程表查到的需要同步记录的key字段的值
  ...
  // 准备插入本地表数组的SQL语句
  // insert into T_ZHOBTMIND2(stid, ddatetime, t, p, u, wd, wf, r, vis, upttime, recid) select obtid, ddatetime, t, p, u, wd, wf, r, vis, upttime, keyid from LK_ZHOBTMIND1 where obtid in (:1, :2, :3);
