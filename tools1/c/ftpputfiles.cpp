...
struct st_arg{
  ...
  int ptype;  // 上传后客户端文件的处理方式：1.什么也不做 2.删除 3.备份 如果是3还要指定备份的目录。
  char localpathbak[301]; // 文件上传后，客户端文件的备份目录，此参数只有当ptype = 3时才有效
  char okfilename[301]; // 已上传成功文件名清单，此参数只有当ptype=1时才有效
  int timeout; // 进程心跳的超时时间
  char pname[51]; // 进程名，建议使用“ftpputfiles_后缀”的方式
}starg;
...
vector<st_fileinfo> vlistfile1; // 已上传成功文件名的容器，从okfilename中加载
vector<st_fileinfo> vlistfile2; // 存放上传前列出客户端文件名的窗口，从nlist文件中加载
vector<st_fileinfo> vlistfile3; // 本次不需要上传的文件容器
vector<st_fileinfo> vlistfile4; // 本次需要上传的文件容器
...
void _help(){
    ...
    printf("Sample:/project/tools1/bin/procctl 30 /project/tools1/bin/ftpputfiles /log/idc/ftpputfiles_surfdata.log \"<host>172.29.193.250</host><mode>1</mode><username>wuhm</username><password>whmhhh1998818</password><localpath>/tmp/idc/surfdata</localpath><remotepath>/tmp/ftpputtest</remotepath><matchname>SURF_ZH*.json</matchname><ptype>1</ptype><localpathbak>/tmp/idc/surfdatabak</localpathbak><okfilename>/idcdata/ftplist/ftpputfiles_surfdata.xml</okfilename><timeout>80</timeout><pname>ftpputfiles_surfdata</pname>\"\n\n\n");
    ...
    printf("<matchname>SURF_ZH*.json</matchname> 待上传文件匹配的规则。"\
                    "不匹配的文件不会被上传，本字段尽可能设置准确,不建议用*匹配全部的文件。\n");
    printf("<ptype>1</ptype> 文件上传成功后，本地文件的处理方式：1.什么也不做 2.删除 3.备份 如果是3还要指定备份的目录。\n");
    printf("<localpathbak>/tmp/idc/surfdatabak</localpathbak> 文件上传后，服务器文件的备份目录，此参数只有当ptype = 3时才有效。\n");
    printf("<okfilename>/idcdata/ftplist/ftpputfiles_surfdata.xml</okfilename> 已上传成功文件名清单，此参数只有当ptype=1时才有效。\n");
    printf("<timeout>80</timeout> 上传文件超时时间，单位：秒，视文件大小和网络带宽决定。\n");
    printf("<pname>ftpputfiles_surfdata</pname> 进程名，尽量采用易懂的，与其他进程不同的名称。方便故障排查。\n\n\n");
...
bool _xmltoarg(char *strxmlbuff){
  ...
  GetXMLBuffer(strxmlbuff, "matchname", starg.matchname, 100); // 待上传文件匹配的规则。
  if(strlen(starg.matchname) == 0){
    logfile.Write("matchname is null.\n");
    return false;
  }

  GetXMLBuffer(strxmlbuff, "ptype", &starg.ptype); // 上传后客户端文件的处理方式：1.什么也不做 2.删除 3.备份 如果是3还要指定备份的目录。
  if((starg.ptype != 1) && (starg.ptype != 2) && (starg.ptype != 3)){
    logfile.Write("ptype is error.\n");
    return false;
  }

  GetXMLBuffer(strxmlbuff, "localpathbak", starg.localpathbak, 300); // 文件上传后，客户端文件的备份目录，此参数只有当ptype = 3时才有效。
  if((starg.ptype == 3) && (strlen(starg.localpathbak) == 0)){
    logfile.Write("localpathbak is null.\n");
    return false;
  }

  GetXMLBuffer(strxmlbuff, "okfilename", starg.okfilename, 300); // 已上传成功文件名清单，此参数只有当ptype=1时才有效
  if((starg.ptype == 1) && (strlen(starg.okfilename) == 0)){
    logfile.Write("okfilename is null.\n");
    return false;
  }

  GetXMLBuffer(strxmlbuff, "timeout", &starg.timeout); // 上传文件超时时间，单位：秒，视文件大小和网络带宽决定
  if(starg.timeout == 0){
...
bool LoadLocalFile(){
  ...
  // 不包含子目录
  // 注意：如果本地目录下的总文件数超过10000，增量上传文件将有问题
  // 建议使用deletefiles程序及时清理本地目录中的历史文件
...
bool LoadOKFile(){
  ...
  CFile File;
  // 注意：如果程序是第一次上传，okfilename是不存在的，并不是错误，所以也要返回true
